...
memberlimit = 2
cr = '*YUKINIKO BOT*'
vz = '*Terima Kasih Sudah Daftar Di YUKINIKO*'
...
*/
const ownerNumber = ["62895426690109@s.whatsapp.net"]
const frendsowner = ["62895426690109@s.whatsapp.net"]
const vip = ["62895426690108@s.whatsapp.net"]
const botName = 'ASSASIN-BOT'
const ownerName = 'Bowo'
const apiKey = 'Apikey'
const apikey = 'O8mUD3YrHIy9KM1fMRjamw8eg'
const vhtear = 'ApiKey'
const zeksapi = '5abP72i9V8iIE6Zqh4IPduCLvcL'
/*
]=====> LOAD JSON <=====[
*/
...
*/
	...
	yuki.on('credentials-updated', () => {
		fs.writeFileSync('./Yukiniko.json', JSON.stringify(yuki.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'Login Info Updated')
	})
	fs.existsSync('./Yukiniko.json') && yuki.loadAuthInfo('./Yukiniko.json')
	yuki.on('connecting', () => {
		start('2', 'Connecting...')
	})
	yuki.on('open', () => {
		...
		try {
			...
			const taukah = mek.message.conversation
				...
				only: {
					...
					public: '☒ Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup Yukiniko, ketik .yukigroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *.iklan*! ',
			...
			const tescuy = '6287821505382@s.whatsapp.net'
...
anu = await fetchJson(`https://api.zeks.me/api/family100?apikey=5abP72i9V8iIE6Zqh4IPduCLvcL`, {method: 'get'})
reply('mohon tunggu sebentar...')
buffer = await getBuffer(anu.string.array)
yuki.sendMessage(from, buffer, 'hai ($name)', { quoted:mek })
...
if (isBanned) return reply(mess.only.benned)    
if (!isUser) return reply(mess.only.userB)
costum( '*YUKINIKO GROUP*\n\n*BOTWA ABAD 1*\nhttps://chat.whatsapp.com/JRlnBMbOvxq9Y0xzBJGgfI\n\n*BOTWA ABAD 2*\nhttps://chat.whatsapp.com/DeuSzylhrP16nfXaLoJ88N', text, tescuk, cr)
break
case 'rules':
if (isBanned) return reply(mess.only.benned)    
if (!isUser) return reply(mess.only.userB)
...
await reply(`────[ YUKINIKO]────\n───[LIMIT STORE]───\n\n⌲ Pengirim : Owner\n⌲ Penerima : ${pushname2}\n\n⎚ Harga Limit : ${koinPerlimit}\n⎚ Limit Dibeli : ${payout}\n⎚ Saldo Yukipay : ${checkATMuser(sender)}\n⎚ Kembalian : ${totalpayout}\n\n${createSerial(15)}\n║▌┃█║▌┃ █║▌┃█┃║▌║\n║▌┃█║▌┃ █║▌┃█┃║▌║`)
...
break
                ...
                addKoinUser('6287720646259@s.whatsapp.net', fee)
			...
			if (isBanned) return reply(mess.only.benned)    
                ...
                if (!isOwner) costum(`╭┤LEVEL│\n├ *Name: ${pushname2}*\n*├ User XP :  ${userXp}*\n├ *Level : ${userLevel}*\n╰══──┤YUKINIKO├──══╯`, text, tescuk,`*Level Mu ${userLevel}*`)
              if (isOwner)  costum (`╭┤LEVEL│\n├ *Name: ${pushname2}*\n*├ User XP :  9999999*\n├ *Level : 9999*\n╰══──┤YUKINIKO├──══╯`, text, tescuk,`*Level Mu 9999*`)
                .catch(async (err) => {
                        console.error(err)
                        await reply(`Error!\n${err}`)
                    })
                ...
                if (!isOwner) return costum( `╭┤DUIT│\n├ *Nama : ${pushname2}*\n├ Rp.${dojar}\n╰══──┤YUKINIKO├──══╯`, text, tescuk,`Duit Mu ${dojar}`)
                if (isOwner) return costum(`╭┤DUIT│\n├ *Nama : ${pushname2}*\n├ Rp.99999\n╰══──┤YUKINIKO├──══╯`, text, tescuk,`Duit Mu 99999`)
                .catch(async (err) => {
                        console.error(err)
                        await reply(`Error!\n${err}`)
                    })
            ...
            default:
                  ...
                  if (budy == 'Cekprefix') {
                  reply(`*YUKINIKO BOT USING PREFIX [ ${prefix} ]*`)
                  }
